...
The other `config.txt` change, also `[pi4]`, is `arm_freq=1800` — raising the ARM cores' ceiling from the 1.5 GHz stock value, at stock voltage (no `over_voltage=`). Nothing else moves: `arm_freq_min` stays at its 600 MHz default, and `core_freq`/`gpu_freq`/`sdram_freq`/`temp_limit` are untouched. 1750 was the original target, but `raspberrypi-cpufreq` rounds any `arm_freq` up to the nearest 100 MHz OPP step it builds from the firmware's reported min/max (`RASPBERRYPI_FREQ_INTERVAL` in the driver), so 1750 became 1800 — confirmed against upstream source, not guessed. Idle behaviour is unaffected: a `time_in_state` sample taken after boot-time services settled showed 98.3% of ticks still at the 600 MHz floor, matching (and slightly beating) the existing idle baseline above. Stability was checked with an 8-hour unattended `stress-ng --cpu 4 --cpu-method all --verify --vm 2 --vm-bytes 75% --verify` soak on a Pi 4B rev 1.1: zero verify failures, no undervoltage/throttle messages in `dmesg`/the kernel journal at any point, and a peak temperature of 78.4 °C — comfortably under the 85 °C firmware soft-throttle limit, on the same heatsink as the `core_freq` measurement above. Core voltage (`hwmon0`/`rpi_volt`, `in0`) moved from 850 mV idle to 904 mV under full 1800 MHz load, confirming the firmware still scales voltage with load dynamically rather than sitting at a fixed rail.
